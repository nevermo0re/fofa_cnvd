...
def fofa_search(kjgs):
    ...
    time.sleep(random.randint(5, 8) + random.random())  # fofa限制请求速率，也为了避免被封
    json_result = json.loads(result.content)
    print(json_result)
    ip_count = json_result['distinct']['ip']
    if ip_count < 20:
        print("公司名："+kjgs + " ---> 独立IP总数：" + str(ip_count))
        with open(r'result1.txt', 'a+') as f:
            f.write('\n')
            f.write('\n')
            f.write("公司名："+kjgs + " ---> 独立IP总数：" + str(ip_count))
            f.close()
    else:
        ...
        with open(r'result1.txt', 'a+') as f:
            ...
            f.write('\n')
            f.write("公司名："+kjgs + " ---> 独立IP总数：" + str(ip_count)+"\n")
        for i in range(len(json_result['aggs']['title'])):
            title = json_result['aggs']['title'][i]['name']
            title_count = json_result['aggs']['title'][i]['count']
            with open(r'result1.txt', 'a+') as f:
                f.write( " ---> 标题榜首：" + title + " ---> 标题对应数：" + str(title_count))
                f.write('\n')
                f.close()
...
if __name__ == '__main__':
    # 打开公司列表，获取公司名称
    print("开始收集--------")
    in_file = open("gs.txt", "rb")
    lines = in_file.readlines()
    print("要爬取的公司数为：", len(lines))
    searchcount=0
    for t in open('gs.txt', 'rb'):
        gs = str(t, "utf-8")
        gs = gs.strip()
        searchcount+=1
        print("当前进度为："+str(searchcount)+"/",len(lines))
        # 去除科技、技术、股份、有限公司等字符
        try:
